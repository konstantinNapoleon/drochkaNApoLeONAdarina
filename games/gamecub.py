...
"📙": [
    "<b>📙 Наследие олда</b>\nПотрёпанная временем книга, пропитанная вайбом тех времён, когда в проект заходили не за наградами, а по зову души и приколу. Новички смотрят с уважением, олды — с флешбеками.",
    "<b>📙 Наследие олда</b>\nРедкий артефакт, доступный только тем, кто помнит времена, когда всё было криво, косо, но по-настоящему лампово. Говорят, если открыть её, можно услышать эхо старого чата.",
    "<b>📙 Наследие олда</b>\nСимвол ветерана проекта. Для новичков — просто книжка. Для олдов — реликвия эпохи багов, мемов и великого выживания без нормальных обнов."
  ],
    "📗": [
...
USE_VIDEOS = {
    ...
    "🇫🇲": [
        "BAACAgIAAyEFAATjGfqtAAIEVGoDZrCUo0-8I3jYmz53Y8XIzd6XAAKOpgACYtgZSHNNzUSlrmqnOwQ"
        ],
    "🏴‍☠️": [
...
async def process_item_use(message: types.Message, item_emoji: str, get_user, save_db):
    ...
    # --- НАСЛЕДИЕ ОЛДА (📙) ---
    if item_emoji == "📙":
        # Получаем список вариантов из словаря
        responses = USE_RESPONSES.get("📙")
        # Выбираем случайный текст
        text = random.choice(responses) if isinstance(responses, list) else responses
        # Предмет НЕ списываем, save_db не вызываем
