...
async def process_droch(message: types.Message, get_user, save_db):
  ...
  # Проверка на первую дрочку (глобальную или в этом чате)
  if chat_stats["masturbations_count"] == 1 and "first_droch" not in user["achievements"]:
    user["achievements"].append("first_droch")
    await message.answer("🎊 **НОВОЕ ДОСТИЖЕНИЕ:** ✊ Первая дрочка!\n└ _Вы сделали это в первый раз!_")
    # Можно добавить бонус за ачивку
    user["balance"] = user.get("balance", 0) + 100

  # ВАЖНО: Добавлен await для сохранения
